...
class MyDataset(Dataset):
    def __init__(self, scene_directory):
        ...
        for scene in range(len(list)):
            expo_path = os.path.join(
                scene_directory, list[scene], 'exposure.txt')
            file_path = list_all_files_sorted(
                os.path.join(scene_directory, list[scene]), '.tif')
    ...
    def __getitem__(self, idx):
        ...
        im1 = output0[x:x + crop_size, y:y + crop_size,
                      :].astype(np.float32).transpose(2, 0, 1)
        im2 = output1[x:x + crop_size, y:y + crop_size,
                      :].astype(np.float32).transpose(2, 0, 1)
        im3 = output2[x:x + crop_size, y:y + crop_size,
                      :].astype(np.float32).transpose(2, 0, 1)
        im4 = label[x:x + crop_size, y:y + crop_size,
                    :].astype(np.float32).transpose(2, 0, 1)
